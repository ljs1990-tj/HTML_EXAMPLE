...
( function ( $ ) {
    ...
    $.extend( $.jqx._jqxDataTable.prototype, {
        ...
        createInstance: function ( args ) {
            var that = this;
            if ( $.jqx.utilities.scrollBarSize != 13 ) {
            ...
            that.host.addClass( that.toTP( 'jqx-datatable' ) );
                        ...
                        for ( var i = 0; i < records.length; i++ ) {
                            ...
                            if ( records[ i ].records && ( records[ i ][ names.expanded ] || records[ i ][ names.leaf ] ) ) {
                                if ( records[ i ]._visible !== false ) {
                                    ...
                                    if ( that.treeGrid && $( that.treeGrid ).jqxTreeGrid( 'showSubAggregates' ) ) {
                                        if ( level != 0 ) {
                                            count--;
                                        }
                                    }
        ...
        exportData: function ( datatype ) {
            ...
            var that = this;
            $cell.removeClass( that.toThemeProperty( 'jqx-grid-cell-selected' ) );
            $cell.removeClass( that.toThemeProperty( 'jqx-fill-state-pressed' ) );
            $cellalt.removeClass( that.toThemeProperty( 'jqx-grid-cell-selected' ) );
            $cellalt.removeClass( that.toThemeProperty( 'jqx-fill-state-pressed' ) );
            $cell.removeClass( that.toThemeProperty( 'jqx-grid-cell-hover' ) );
            $cell.removeClass( that.toThemeProperty( 'jqx-fill-state-hover' ) );
            $cellalt.removeClass( that.toThemeProperty( 'jqx-grid-cell-hover' ) );
            $cellalt.removeClass( that.toThemeProperty( 'jqx-fill-state-hover' ) );
            ...
            $.each( columns, function ( index ) {
                ...
                var removeClassFunc = function ( cell ) {
                    cell.removeClass( that.toThemeProperty( 'jqx-cell' ) );
                    cell.removeClass( that.toThemeProperty( 'jqx-grid-cell-selected' ) );
                    cell.removeClass( that.toThemeProperty( 'jqx-fill-state-pressed' ) );
                    cell.removeClass( that.toThemeProperty( 'jqx-grid-cell-hover' ) );
                    cell.removeClass( that.toThemeProperty( 'jqx-fill-state-hover' ) );
        ...
        beginroweditbykey: function ( key, editColumn ) {
            ...
            if ( item ) {
                ...
                for ( var i = 0; i < cells.length; i++ ) {
                    ...
                    var cellContent = "<div style='height:" + height + "px; width: 100%; overflow: hidden; border-radius: 0px; -moz-border-radius: 0px; -webkit-border-radius: 0px; z-index: 9999;'></div>";
            ...
            if ( rowid != undefined ) {
                ...
                var applychanges = function ( that, rowid ) {
                    ...
                    if ( that.source && that.source._knockoutdatasource && !that._updateFromAdapter && that.autokoupdates ) {
                        if ( that.source._source._localdata ) {
                            that.source.suspendKO = true;
                            that.source._source._localdata.pop();
                            that.source.suspendKO = false;
                        }
                    }
                        ...
                        if ( column.pinned ) {
                            ...
                            if ( this.filterrow ) {
                                if ( this.filterrow[ 0 ].cells ) {
                                    var filtercell = this.filterrow[ 0 ].cells[ j ];
                                }
                            }
        ...
        _rendercolumnheaders: function () {
            ...
            $.each( this.columns.records, function ( i, value ) {
                ...
                var pinnedZIndex = !that.rtl ? 250 + zindex - 1 : 250 + zindex + 1;
