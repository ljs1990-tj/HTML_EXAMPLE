...
( function ( $ ) {
    $.extend( $.jqx._jqxGrid.prototype, {
        ...
        _exportData: function ( dataFormat, dataExport ) {
            var that = this;
            ...
            for ( var i = 0; i < that.columns.records.length; i++ ) {
                if ( that.columns.records[ i ].datafield !== null ) {
                    cols[ index++ ] = { label: that.columns.records[ i ].text, dataField: that.columns.records[ i ].datafield };
                }
            }
            ...
            if ( !dataExport.groupBy && that.groups ) {
        ...
        exportview: function ( datatype, filename ) {
            ...
            that._exportData( datatype, filename );
        },
